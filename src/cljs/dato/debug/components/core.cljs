                ...
                (dom/div
                  (case (:tool l-state)
                    ...
                    :timeline   (dom/div
                                  ...
                                  (dom/button {:on-click #(let [session-id (js/prompt "Enter a session id")]
                                                            (dato/rpc-call
                                                             dato
                                                             ...
                                                             (fn [history]
                                                               (om/set-state!
                                                                owner
                                                                :log-override
                                                                (reduce (fn [acc history-item]
                                                                          (js/console.log (:tx-data history-item))
                                                                          (let [transaction (mapv dsu/datom->transaction (:tx-data history-item))]
                                                                            (js/console.log "tx" transaction)
                                                                            (js/console.log "acc" acc)
                                                                            (if (seq acc)
                                                                              (conj acc {:tx (d/with (:db-after (:tx (last acc)))
                                                                                                     transaction)})
